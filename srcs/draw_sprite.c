...
static void		print_sprite_3d(t_context *ct,
	float distance, float sp_position_angle, int id)
{
	SDL_Rect	dst;
	int			sp_height;
	float		delta_angle;
	...
	if (ft_float_abs(delta_angle) > 150 && sp_position_angle < ct->cam.angle)
		...
		delta_angle = sp_position_angle - 360 - ct->cam.angle;
	dst = define_rect((int)(ct->xwin / 2 - ct->xwin * delta_angle /
		ANGLE - sp_height / 2), (int)(ct->ywin / 2), sp_height, sp_height);
	if (id % 10 == KEY_CUBE)
...
static void		draw_one_sprite_in_3d(t_context *ct, t_sp_lst *lst)
{
	...
	sp_position_angle = convert_rad_to_deg(atan2((lst->posi.y - ct->cam.posi.y)
		* (-1), (lst->posi.x - ct->cam.posi.x)));
	sp_position_angle = angle_limit(sp_position_angle);
	wall_dis = dda_return_distance(ct, sp_position_angle);
	near_wall_dis_down = dda_return_distance(ct,
		angle_limit(sp_position_angle + 3.0));
	near_wall_dis_up = dda_return_distance(ct,
		angle_limit(sp_position_angle - 3.0));
	if (wall_dis < 0 || (wall_dis > lst->distance && near_wall_dis_up >
		lst->distance && near_wall_dis_down > lst->distance))
		print_sprite_3d(ct, lst->distance, sp_position_angle, lst->id);
}

int				walk_on_sprite(t_context *ct, t_floatpoint posi_sp)
{
	return ((((int)posi_sp.x) == ((int)ct->cam.posi.x)
		&& ((int)posi_sp.y) == (int)ct->cam.posi.y));
}
